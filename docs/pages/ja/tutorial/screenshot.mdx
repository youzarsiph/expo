...
export default function Index() {
  /* @tutinfo フックから権限のレスポンスと `requestPermission()` メソッドをインポートするため、この文を追加します。 */const [permissionResponse, requestPermission] = ImagePicker.useMediaLibraryPermissions();/* @end */
  // ...rest of the code remains same

  /* @tutinfo 権限の状態を確認するため、if 文を追加します。requestPermission() メソッドは、ユーザーが権限を許可または拒否するためのダイアログを表示します。 */
  useEffect(() => {
...
import { useState, /* @tutinfo `react` から `useRef` フックをインポートします。 */useRef/* @end */ } from 'react';
/* @tutinfo `react-native-view-shot` から `captureRef` をインポートします。 */import { captureRef } from 'react-native-view-shot';/* @end */

export default function Index() {
  /* @tutinfo imageRef 変数を作成します。 */ const imageRef = useRef<View>(null);/* @end */
  ...
  return (
    <GestureHandlerRootView style={styles.container}>
      <View style={styles.imageContainer}>
        /* @tutinfo ImageViewer と EmojiSticker をラップする View コンポーネントを追加します。 */<View ref={imageRef} collapsable={false}>/* @end */
...
また、`captureRef()` メソッドはスクリーンショットの URI で解決される Promise を返します。この URI を [`MediaLibrary.saveToLibraryAsync()`](/versions/v54.0.0/sdk/media-library/#medialibrarysavetolibraryasynclocaluri) のパラメータとして渡し、デバイスのメディアライブラリにスクリーンショットを保存します。
...
```tsx app/(tabs)/index.tsx|collapseHeight=440
import * as ImagePicker from 'expo-image-picker';
/* @tutinfo `expo-media-library` から `MediaLibrary` をインポートします。 */import * as MediaLibrary from 'expo-media-library';/* @end */
...
export default function Index() {
  ...
  /* @tutinfo スクリーンショットを撮影して画像を保存するコードに、コメントを置き換えます。 */
